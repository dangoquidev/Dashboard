...
import { getPuuidFromInGameName, getInfoFromPuuid, getFiveLastGame, getMatch, getMatchInfoFromResponse } from './riot.services';
import { MatchInfo } from './riot.type';

export const getAccountByInGameName = async (req: express.Request, res: express.Response) => {
    ...
    games.map(async (matchId: string) => {
        ...
        matchHistoryInfo.push(await getMatchInfoFromResponse(data, account.puuid));
    })

    return res.status(200).json({ success: true, account, info, matchHistoryInfo });
}